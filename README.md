...
- GPS 推送：將 `lat` / `lng` 傳送到手機端 MacroDroid `/gps`；手機 HTTP 傳送使用背景 worker，timeout 不會阻塞 CSV 每秒紀錄
...
```csv
Timestamp,Latitude,Longitude,Action,Note,TimestampISO,DeltaSeconds,DistanceMeters
```

- `Timestamp`：本地時間，含毫秒。
- `TimestampISO`：ISO 格式時間，含毫秒。
- `DeltaSeconds`：與上一筆實際紀錄的時間差，單位秒。
- `DistanceMeters`：與上一筆實際紀錄座標的距離，單位公尺。

新任務會以真實系統秒數為節拍，每個 wall-clock second 最多紀錄一筆 GPS。手機端 HTTP 傳送若 timeout，只會影響手機更新；CSV 紀錄仍會照節拍寫入。
